...
int main( int argc, char** argv )
{
  ...
  ros::Subscriber odom_sub = n.subscribe("/goal_reached", 10, goalReachCallback);
  bool done = false;
  ...
  while (ros::ok()) {
    ...
    switch (goal_reach_state)
    {
      case 0: // publish pick-up marker
        ...
        case 1: // wait for robot to reach drop-off site
          {
        ...
        case 2:   // publish drop-off marker
          {
            ...
            case 3: // wait for robot to reach drop-off site
          {
            marker.action = visualization_msgs::Marker::ADD;
            // Set the pose of the marker.  This is a full 6DOF pose relative to the frame/time specified in the header
